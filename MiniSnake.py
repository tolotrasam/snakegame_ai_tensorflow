...
def neural_network_model_smaller(input_size, output_size):
    network = input_data(shape=[None, input_size, 1], name='input')
...
def initiate_model(training_data):
    shape_second_parameter = len(training_data[0][0])
    x = np.array([i[0] for i in training_data])
    X = x.reshape(-1, shape_second_parameter, 1)
    y = [i[1] for i in training_data]
    model = neural_network_model_smaller(input_size=len(X[0]), output_size=len(y[0]))
...
if __name__ == "__main__":
    ...
    # training_data = initial_population(None)
    # load from last session
    training_data_load = np.load('saved.npy')
    score_requirement = training_data_load[1]
    training_data = training_data_load[0]

    # creating a dummy model
    model = initiate_model(training_data)

    # load from previous model
    # model.load('miniskake.tflearn')
    model.load('miniskake_trained.tflearn')
    evaluate(model)
    exit(0)
    ...
    while True:
        ...
        training_data = np.append(training_data, initial_population(None), axis=0)
